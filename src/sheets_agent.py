...
def get_transactions_from_spreadsheet(spreadsheet):
    """Read transaction rows from all month worksheets in a spreadsheet."""
    transactions = []
    for worksheet in spreadsheet.worksheets():
        ...
        for row in worksheet.get_all_records():
            if any(str(value).strip() for value in row.values()):
                transactions.append({**row, "Month": worksheet.title})
